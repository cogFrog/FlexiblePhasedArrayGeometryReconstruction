...
for frac_bw in fractional_bws:
    ...
    csv = rf.RangeFinderCSV(10e9, bw, points, True)

    # calibrate at 100 mm or 0.1 m
    csv.find_range(True, 0.1, f'results\\distanceResultsOldAnt\\dist_100mm.csv', 1e9)

    actual_dist = np.array([*range(20, 191, 10)])
    estimated_dist_fsk = []

    for dist in range(20, 191, 10):
        estimated_dist_fsk.append(csv.find_range(False, 0, f'results\\distanceResultsOldAnt\\dist_{dist}mm.csv',1e9)[0] * 1000)
